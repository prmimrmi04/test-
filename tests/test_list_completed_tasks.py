...
# Ensure the root is in sys.path so scripts can be imported
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from scripts.list_completed_tasks import list_completed_tasks

def test_list_completed_tasks(tmp_path, capsys):
    ...
    # Run the function
    list_completed_tasks(str(tasks_file))

    # Check output
    captured = capsys.readouterr()
    output = captured.out.strip().split("\n")
    assert output == ["Task 1", "Task 4"]

def test_list_completed_tasks_file_not_found(capsys):
    list_completed_tasks("nonexistent.json")
    captured = capsys.readouterr()
    assert "Error: File 'nonexistent.json' does not exist." in captured.err

def test_list_completed_tasks_invalid_json(tmp_path, capsys):
...
def test_list_completed_tasks_not_array(tmp_path, capsys):
